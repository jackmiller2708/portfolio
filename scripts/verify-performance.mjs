...
async function runLighthouse() {
  ...
  for (const route of routes) {
    ...
    if (result.accessibility < 90) issues.push(`${route}: Accessibility ${result.accessibility} < 90`);
    if (result.bestPractices < 90) issues.push(`${route}: Best Practices ${result.bestPractices} < 90`);
